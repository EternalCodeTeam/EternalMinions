    ...
    public void connect() {
        try {
            ...
            String jdbcUrl = switch (type) {
                // H2 rejects a bare relative path in the URL ("plugins/EternalMinions") and an absolute
                // Windows path breaks the "./" prefix, so the folder is always resolved to absolute first.
                case H2, SQLITE -> type.formatUrl(dataFolder.getAbsoluteFile());
